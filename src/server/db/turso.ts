...
// La bitácora no expone operaciones de edición ni borrado: las filas son append-only.
async function ensureAuditLogsTable() {
  if (!cachedClient) return;
  await cachedClient.execute(`CREATE TABLE IF NOT EXISTS audit_logs (
    id INTEGER PRIMARY KEY,
    actor_user_id INTEGER,
    actor_email TEXT NOT NULL,
    action TEXT NOT NULL,
  ...
  await cachedClient.execute('CREATE INDEX IF NOT EXISTS idx_audit_logs_created_at ON audit_logs(created_at DESC)');
  await cachedClient.execute('CREATE INDEX IF NOT EXISTS idx_audit_logs_entity ON audit_logs(entity_type, entity_id)');
  await cachedClient.execute('CREATE INDEX IF NOT EXISTS idx_audit_logs_actor ON audit_logs(actor_user_id, created_at DESC)');
}
...
async function initializeSchema() {
  ...
  try {
    // En lugar de hacer split manual, enviamos bloques más seguros o usamos transacciones
    // Pero para solucionar el error de las comillas ahora mismo:
    
    // Ejecutamos el SCHEMA_SQL completo como una sola operación de batch si el driver lo permite,
    // o limpiamos el split para que no rompa con los textos largos.
    
    await cachedClient.batch(
      SCHEMA_SQL.split(';').filter(s => s.trim().length > 4).map(s => s.trim()),
      "write"
    );

    await cachedClient.batch(
      INITIAL_DATA_SQL.split(';').filter(s => s.trim().length > 4).map(s => s.trim()),
      "write"
    );
    ...
    // No lanzamos el error para evitar que la app explote, pero lo logueamos
  }
}
