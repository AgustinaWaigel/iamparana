...
export function FormacionCardsGrid({ uploadedDocuments, uploadedLinks, resourcePages }: FormacionCardsGridProps) {
  ...
  const cards = useMemo<CardItem[]>(() => {
    ...
    const linkCards: CardItem[] = linksState.map((resourceLink) => ({
      id: `link-${resourceLink.id}`, kind: 'link', title: resourceLink.title, description: resourceLink.description || 'Enlace compartido por el equipo de formación.', href: resourceLink.url, badge: 'Enlace', accent: 'yellow', resourceId: resourceLink.id, linkUrl: resourceLink.url, thumbnailUrl: resourceLink.thumbnail_url || null,
    }));

    const resourcePageCards: CardItem[] = resourcePagesState.map((page) => ({
      id: `resource-page-${page.id}`, kind: 'resource-page', title: page.title, description: page.description || 'Página de recursos con secciones y contenido.', href: `/formacion/recursos/${page.slug}`, badge: 'Página de formación', accent: 'yellow', resourceId: page.id, thumbnailUrl: page.thumbnail_url || page.texture_url || null,
    }));
