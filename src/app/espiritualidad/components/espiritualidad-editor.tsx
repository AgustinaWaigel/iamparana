...
interface EspiritualidadEditorProps {
  isAdmin: boolean;
  onRefresh?: () => void;
}
  ...
  const handleSubmitLink = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/admin/links', {
        ...
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ section: 'espiritualidad', title: linkData.title, description: linkData.description || null, url: linkData.url, icon: linkData.icon, thumbnailUrl: linkThumbnailUrl }),
      });
    ...
    try {
      ...
      const response = await fetch('/api/admin/resource-pages', {
        ...
        body: JSON.stringify({
          ...
          template: 'purple', // El template ahora es solo estética
          section: 'espiritualidad' // <--- ESTA es la clave lógica
        }),
      });
      ...
      setPageData({ title: '', slug: '', description: '', textureUrl: '/assets/textures/espiritualidad.webp' });
                  ...
                  <div className="space-y-5 animate-in fade-in zoom-in-95 duration-300">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className={inputClass}
                        placeholder="/assets/textures/espiritualidad.webp"
                      />
                    </div>
