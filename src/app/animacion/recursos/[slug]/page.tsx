...
import { ExternalLink, FileText, FolderKanban, Link as LinkIcon, Pencil, Trash2 } from 'lucide-react';
...
const TEMPLATE_MAP: Record<string, { banner: string; accent: string; bgAccent: string; border: string; cardHover: string; }> = {
  gold: {
...
export default async function ResourcePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const theme = TEMPLATE_MAP[data.page.template] || TEMPLATE_MAP.earth;
    ...
    <main className="pb-24 bg-[#F9F9F8] min-h-screen font-sans">
      <div className={`relative min-h-[34vh] sm:min-h-[38vh] md:min-h-[50vh] flex items-center justify-center overflow-hidden py-12 ${theme.banner}`}>
        <div
          ...
          style={{
            backgroundImage: `url(${data.page.texture_url || '/assets/textures/areasg.webp'})`,
...
function ResourceItemCard({
  ...
  href: string | null;
  type: 'doc' | 'link';
  theme: { accent: string; bgAccent: string; border: string; cardHover: string };
  isAdmin: boolean;
}) {
  ...
  return (
    <div className={`group relative flex flex-col justify-between bg-white rounded-2xl border border-stone-200 p-5 transition-all duration-300 shadow-sm ${theme.cardHover} hover:-translate-y-1`}>
      ...
      {description && <p className="text-sm text-stone-500 line-clamp-2 mb-5 leading-relaxed">{description}</p>}

      <a
        href={href || '#'}
        target="_blank"
        rel="noopener noreferrer"
        className={`mt-auto flex items-center justify-center gap-2 w-full py-2.5 rounded-xl text-xs font-black uppercase tracking-widest border transition-all ${theme.bgAccent} ${theme.accent} border-transparent hover:bg-white ${theme.border}`}
      >
